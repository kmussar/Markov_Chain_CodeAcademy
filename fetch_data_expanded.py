...
# Find the first 10 characters listed in a single movie before moving to all movies in the dictionary.
character_lst = []
page = requests.get('https://www.imdb.com/title/tt0848228/?ref_=fn_al_tt_1')
soup = BeautifulSoup(page.content, 'html.parser')
soup.find_all(class_="character")
# load the first y number of characters from each movie
for y in range(6): 
    char_name = soup.find_all(class_="character")[y].get_text()
